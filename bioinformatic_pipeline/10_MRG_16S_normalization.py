...
MRG_DIR = "/your/path/MRG_abundance/coverage"      # *_bacmet_counts.tsv
S16_DIR = "/your/path/16S_coverage"                # *_16S_counts.tsv
OUT_MATRIX = "/your/path/MRG_per16S_matrix.tsv"
...
############################################
# 2) READ MRG COUNTS AND NORMALISE TO 16S
############################################

print(">>> Reading MRG coverage from:", MRG_DIR)

matrix = defaultdict(lambda: defaultdict(float))
samples_set = set()
genes_set = set()

for path in glob.glob(os.path.join(MRG_DIR, "*_bacmet_counts.tsv")):
    with open(path, newline="") as fh:
        reader = csv.DictReader(fh, delimiter="\t")
...
if not matrix:
    raise SystemExit(
        "No MRG_per16S values calculated. Check paths and input headers."
    )

samples = sorted(samples_set)
genes = sorted(genes_set)

print(f"Number of MRGs (BacMet_ID): {len(genes)}")
print(f"Number of samples with MRG_per16S: {len(samples)}")
...
with open(OUT_MATRIX, "w", newline="") as fh:
    writer = csv.writer(fh, delimiter="\t")
    writer.writerow(["BacMet_ID"] + samples)

    for gene in genes:
        row = [gene]

        for sample in samples:
            row.append(f"{matrix[gene].get(sample, 0.0):.6g}")

        writer.writerow(row)

print(f">>> Written MRG_per16S matrix to: {OUT_MATRIX}")
print("Done.")
